...
public class AsciiImage
{
	...
	{
		//Bildhöhe vergleichen:
		if(((AsciiImage)o).getHeight()!=this.hoehe)
		{
			return false;
		}
		//Bildbreite vergleichen:
		if(((AsciiImage)o).getWidth()!=this.breite)
		{
			return false;
		}
		//Gesamtes Bild durchlaufen:
		for(int y=0; y<hoehe; y++)
		{
			for(int x=0; x<breite; x++)
			{
				...
				if(((AsciiImage)o).getPixel(x,y)!=this.getPixel(x,y))
				{
					return false;
				}
			}
		...
		for(int y=0; y<hoehe; y++)
		{
			for(int x=0; x<breite; x++)
			{
				...
				if(unique==true)
				{
					...
					uniquechar[counter]=getPixel(x,y);
					counter++;
				}
			}
		}
